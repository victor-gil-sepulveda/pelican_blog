import os, inspect, datetime, clip
...
@shopping.subcommand(description='Creates a new post using the current timestamp')
...
def make_entry(title, draft):
    ...
    t = load_template("new_post.template").strip().format(title=title,
                                hashes='#' * len(title),
                                year=today.year,
                                month=today.month,
                                day=today.day,
